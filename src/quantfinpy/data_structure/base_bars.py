...
class BaseBars(ABC):
    ...
    def _batch_iterator(self, file_path_or_df: Union[str, Iterable[str], pd.DataFrame]) -> Generator[pd.DataFrame, None, None]:
        ...
        if isinstance(file_path_or_df, (list, tuple)):
            ...
            raise ValueError('Invalid input type for file_path_or_df. Expected a string (path to a csv file), an iterable of strings, or a pd.DataFrame.')
    ...
    def _assert_csv(test_batch: pd.DataFrame):
        ...
        try:
            ...
            raise ValueError('Date-time column in CSV file is not in a valid format:',
                             test_batch.iloc[0, 0])
...
class BaseImbalanceBars(BaseBars):
    ...
    def __init__(self, immb_bar_type: str, batch_size: int,
...
class BaseRunBars(BaseBars):
    ...
    def __init__(self, run_bar_type: str, batch_size: int, num_prev_bars: int,
                 expected_imbalance_window: int,
